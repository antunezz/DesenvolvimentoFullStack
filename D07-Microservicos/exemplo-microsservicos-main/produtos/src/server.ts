...
const init = async () => {
  ...
  await consul.catalog.service.nodes({service: 'produtos'}).then((result) => {
    if (Array.isArray(result)) {
      result.forEach(async node => {
        await consul.agent.service.deregister({ id: node.ServiceID });
        console.log('Desregistrando produtos', node.ServiceID, node.ServicePort);
      });
    }
  });
  await consul.agent.service.register(opts).then(async (a) => {
    console.log(`Registrado no consul com id: ${consulId}`);
  }).catch((e) => { 
    console.log('Falha ao registrar', e);
  });
};
